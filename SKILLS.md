...
|---|---|---|---|
| `cv-tailoring` | 1.12.1 | Tailors a CV to a job description using an 8-phase workflow: intake → research → gap assessment → discovery → ATS scoring → generation → validation/QA → summary. Saves DOCX to a local output folder; no cloud sync. | [README](plugins/cv-tailoring/README.md) · [CHANGELOG](plugins/cv-tailoring/CHANGELOG.md) |
