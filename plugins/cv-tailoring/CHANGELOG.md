# Changelog — cv-tailoring

All notable changes to the `cv-tailoring` plugin/skill are documented here.
This file versions independently of other skills in this marketplace — see
...
next), so this repo moved from one shared version number to per-plugin
versioning — each plugin now owns its own release cadence.

### Housekeeping
- This `CHANGELOG.md` moved from the repo root to
  `plugins/cv-tailoring/CHANGELOG.md` — it only ever documented cv-tailoring
  anyway. The repo root now has its own `CHANGELOG.md` for marketplace-level
  changes only (new skills added, shared tooling, manifest schema) —
  cv-tailoring's version no longer needs to track it.
- Added `plugins/cv-tailoring/README.md` — the plugin's own detailed
  writeup (description, workflow modes, install instructions), moved out
  of the repo root `README.md`, which is now a thin marketplace-level index.
- `SKILLS.md` is now a thin catalog table (one row per skill, links out to
  each plugin's own README/CHANGELOG) instead of embedding full version
  history per skill — that embedding was a third copy of the same history
  already in `CHANGELOG.md`, which is exactly the kind of duplication that
  caused v1.10.0/v1.11.0's staleness in the first place.
- `scripts/check_release_consistency.py` rewritten to be plugin-aware: it
  now loops over every `plugins/*/`, not just cv-tailoring, checking each
  plugin's `plugin.json` version against its own `CHANGELOG.md`, its own
  `README.md` header, its `marketplace.json` entry, and its row in
  `SKILLS.md`.
- Going forward, git tags for this plugin use the `cv-tailoring-vX.Y.Z`
  format (not bare `vX.Y.Z`) to avoid colliding with other plugins' tags.
  Existing bare tags (`v1.0.0`-`v1.11.1`) predate this convention and are
  left as-is — they're unambiguous today since cv-tailoring was the only
  plugin.
